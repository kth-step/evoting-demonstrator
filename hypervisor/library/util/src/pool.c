...
int pool_init(pool_ctxt_t * ctxt, int item_size, int item_count)
{
	...
	if (item_size < LIST_MIN_ITEM_SIZE)
		item_size = LIST_MIN_ITEM_SIZE;
	if (item_count < 2)
		item_count = 2;
	...
	ctxt->current = 0;
	ctxt->list =
	    (void **)malloc(item_size * item_count +
			    sizeof(void *) * item_count);
	if (!ctxt->list)
		return 0;
