...
void test_cleanup(test_context_t * ctx)
{
	if (ctx->tests_total == 0)
		...
		fprintf(stderr,
			"FAILED: %d tests in %d gorups. %d errors where found\n",
		...
		if (ctx->errors_total > 0) {
			printf
			    ("   [ TEST SUMMARY: %d groups, %d test (FAILED: %d) ]\n",
...
void context_report_group(test_context_t * ctx)
{
	if (ctx->tests_group == 0) {
		...
		fprintf(stderr, "   GROUP '%s': all %03d test%s ok\n",
			ctx->group_name,
			ctx->tests_group, ctx->tests_group > 1 ? "s" : "");
		 ...
		 const char *filename, int line, const char *format, ...)
{
	...
	if (!ctx->group_name)
		fail("Test does not belong to any group");
	...
	if (succ) {
		...
		fprintf(stderr, " ERROR %d: %s:%d: ", ctx->errors_total,
			filename, line);
