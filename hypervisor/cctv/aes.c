...
int main(int argc, char *argv[]) {
	char *path = argv[1];
	FILE *fpr = fopen(path, "r");	//Read and write binary.
	FILE *fpw;	//Read and write binary.
	...
	fread(buffer, 1, BUFFER_SIZE, fpr);	//Read file into memory.
	fclose(fpr);
//	fseek(fp, 0, SEEK_SET);				//Set file pointer to beginning of file.
	...
	fwrite(buffer, 1, BUFFER_SIZE, fpw);	//Write decrypted memory to file.
	fclose(fpw);
	return 0;
}*/
...
int main(int argc, char *argv[]) {
	char *path = argv[1];
	FILE *fp = fopen(path, "rw+");	//Read and write.
	printf("File to decrypt: %s\n", path);
	if (fp == NULL) {
		printf("open file failed\n");
		return -1;
	...
	fread(buffer, 1, BUFFER_SIZE, fp);	//Read file into memory.
	fseek(fp, 0, SEEK_SET);				//Set file pointer to beginning of file.
	decrypt();							//Decrypt.
	fwrite(buffer, 1, BUFFER_SIZE, fp);	//Write decrypted memory to file.
	fclose(fp);
	return 0;
}
