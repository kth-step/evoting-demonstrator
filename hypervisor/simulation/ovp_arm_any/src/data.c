...
static void data_init(struct data_context *ctx, icmProcessorP proc,
		      const char *name)
{
...
static int data_write_memory(struct data_context *ctx, unsigned int adr,
			     unsigned int *data)
{
	return icmDebugWriteProcessorMemory(ctx->proc,
					    (Addr) (0xFFFFFFFF & adr), data,
					    4, 1, ICM_HOSTENDIAN_HOST);
}

static int data_read_memory(struct data_context *ctx, unsigned int adr,
			    unsigned int *data)
{
	return icmDebugReadProcessorMemory(ctx->proc,
					   (Addr) (0xFFFFFFFF & adr), data, 4,
...
void data_save(icmProcessorP proc, const char *name, int adr, int size)
{
	...
	if (data_open_write(&ctx, adr, size)) {
		...
		if (failed) {
			printf
			    ("NOTE: there were memory access errors. Saved data is invalid\n");
		}
	} else
		printf("Unable to create '%s' data file\n", name);
}
...
void data_load(icmProcessorP proc, const char *name, int adr, int has_adr)
{
	...
	if (data_open_read(&ctx)) {
		...
		printf("Loaded %s %08lx %08lx\n", ctx.header.name,
		       ctx.header.start, ctx.header.size);
		if (!has_adr)
		...
		if (failed) {
			printf
			    ("NOTE: there were memory access errors. Memory may be invalid\n");
		}
	} else
		printf("Unable to open '%s' data file\n", name);
}
	...
	if (data_open_read(&ctx)) {
		...
		for (i = 0; i < ctx.header.size; i += 4) {
			...
			if (!data_read_memory(&ctx, adr, &data2))
				...
				printf
				    ("Data changed at 0x%08lx:  OLD=0x%08lx  NEW=0x%08lx",
				     adr, data1, data2);
				...
				if (cnt == 8)
					printf
					    ("... (additional changes not reported)\n");
			}
			adr += 4;
		}
		data_end(&ctx);
		...
		if (cnt > 0) {
			printf
			    (" *** DATA COMPARISION: There were %d changes.\n",
			     cnt);
		} else {
			printf
			    (" *** DATA COMPARISION: Saved data was identical to memory.\n");
		}

		if (failed) {
			printf
			    ("NOTE: there were memory access errors. Comparision my be incorrect.\n");
		}
	} else
		printf("Unable to open '%s' data file\n", name);
