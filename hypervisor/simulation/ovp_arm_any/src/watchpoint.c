...
static void print_watchpoint(icmWatchPointP w, int active)
{
	printf("Watch %08lx-%08lx type=%08lx\n",
	       ...
	       icmGetWatchPointHighAddress(w), icmGetWatchPointType(w)
	    );
		  ...
		  int write_access, int physical)
{
	...
	if (write_access)
		x = icmSetProcessorAccessWatchPoint(processor, phy, low, high,
						    0, 0);
	else
		x = icmSetProcessorReadWatchPoint(processor, phy, low, high,
						  0, 0);
