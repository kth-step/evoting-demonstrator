...
void my_constructor(void *obj, void *ud)
{
	...
	test_not_equal(test, "not already cosntructed", data->data1,
		       MAGIC_ALLOC);
...
void my_destructor(void *obj, void *ud)
{
	...
	test_equal(test, "destructed obj has been consructed", data->data1,
		   MAGIC_ALLOC);
...
void test_do(test_context_t * test_)
{
	...
	test_group_start(test, "get slab items");
	{
		for (i = 0; i < ITEM_COUNT; i++) {
			...
			if (tmp) {
				test_equal(test,
					   "allocated obj has been consructed",
					   tmp->data1, MAGIC_ALLOC);
			}
		}

		count = my_user_data;
		test_larger_or_equal(test,
				     "correct amount of objects constructed",
				     ITEM_COUNT, my_user_data);

		tmp = slab_alloc(&slab);
		test_null(test, "slab must be empty", tmp);
		test_equal(test,
			   "did not construct a new one when slab was empty",
			   count, my_user_data);
	}
	test_group_end(test);
	...
	test_group_start(test, "return slab items");
	{
		...
		test_equal(test,
			   "did not construct new slabs after some free/alloc pairs",
			   count, my_user_data);
	}
	test_group_end(test);
