...
void test_do(test_context_t * test)
{
	...
	test_group_start(test, "remove");
	{
		...
		/* see what is left */
		test_equal(test, "remove: has now 4 items", 4,
			   olist_size_get(&list));
		...
		test_equal(test, "remove: has now 3 items", 3,
			   olist_size_get(&list));
		test_equal(test, "remove: item 3 no longer in there", j,
			   i & ~(1UL << (uint32_t) tmp));
		...
		test_equal(test, "remove: has now 2 items", 2,
			   olist_size_get(&list));
		test_equal(test, "remove: item 2 no longer in there", i,
			   j & ~(1UL << (uint32_t) tmp));
	...
	test_group_start(test, "grow");
	{
		...
		/* see if they are ALL in there: */
		{
			...
			test_not_null(test,
				      "grow: allocated bitset for this test",
				      set);

			bitset_set_all(set, FALSE);
			for (i = 0; i < olist_size_get(&list); i++) {
				tmp = olist_item_get(&list, i);
				test_not_null(test, "grow: item was not null",
					      tmp);

				j = (uint32_t) tmp;

				test_larger_than(test, "grow: item > 0", j, 0);
				test_less_or_equal(test, "grow: item <= 10M",
						   j, 1024 * 1024 * 10);
	...
	test_group_start(test, "invalid access");
	{
		...
		tmp = olist_item_get(&list, 3);
		test_not_null(test, "invalid: item 3 is within valid range",
			      tmp);

		olist_item_pop(&list);
		tmp = olist_item_get(&list, 3);
		test_null(test, "invalid: item 3 is now outside valid range",
			  tmp);
	}
	test_group_end(test);
