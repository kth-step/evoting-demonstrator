...
void check_restored(test_context_t * test, mreg_t * reg)
{
	...
	test_equal(test, "restored: level1 count", level1_count_now,
		   level1_count_org);
	test_equal(test, "restored: level1 size", level1_size_now,
		   level1_size_org);
	test_equal(test, "restored: level2 count", level2_count_now,
		   level2_count_org);
}
...
void test_do(test_context_t * test)
{
	...
	test_group_start(test, "Setup and sanity check");
	{
		...
		test_null(test, "Sanity check: alloc from empty mreg",
			  buffer[0]);
		...
		test_equal(test, "Sanity check: added required region mreg",
			   TRUE, succ);

		get_level1_stats(reg, &level1_count_org, &level1_size_org);
		get_level2_stats(reg, &level2_count_org);

		test_equal(test, "Sanity check: level1 count at start", 1,
			   level1_count_org);
		test_equal(test, "Sanity check: level1 size at start", MB * 3,
			   level1_size_org);
		test_equal(test, "Sanity check: level2 count at start", 2,
			   level2_count_org);
	}
	test_group_end(test);
	...
	test_group_start(test, "alloc (large)");
	{
		...
		test_equal(test, "alloc large: got 1, size 1MB", MB,
			   MSIZE(buffer[0]));
		...
		test_equal(test, "alloc large: got 2, size 1MB", MB,
			   MSIZE(buffer[1]));
		...
		test_equal(test, "alloc large: got 3, size 1MB", MB,
			   MSIZE(buffer[2]));
		...
		test_equal(test,
			   "alloc large: didn't get 3, total still -3MB",
			   level1_size_now, level1_size_org - 3 * MB);
	}
	test_group_end(test);
	...
	test_group_start(test, "alloc (small)");
	{
		...
		test_equal(test, "alloc small: got 1, size 8KB", 8 * KB,
			   MSIZE(buffer[0]));
		...
		test_equal(test,
			   "alloc small: got 1, level 2 count unchanged",
		...
		test_equal(test, "alloc small: got 2, size 8KB", 8 * KB,
			   MSIZE(buffer[1]));
		...
		test_equal(test,
			   "alloc small: got 2, level 2 count unchanged",
			   level2_count_now, level2_count_org);
		...
		test_equal(test, "alloc small: got 3, size 512KB", 512 * KB,
			   MSIZE(buffer[2]));
		...
		test_equal(test, "alloc small: got 4, size 1024KB", MB,
			   MSIZE(buffer[3]));
	...
	{
		reg = mreg_create(PAGE_BITS);
		test_not_null(test, "sanity check: allocated another mreg",
			      reg);
		...
		test_equal(test, "atach multi: total end",
			   5 * MB + 16 * KB - 1, reg->end);
		test_equal(test, "attach multi: level1 count", 2, level1_count_now);	/* 0MB & 1MB are combined */
		test_equal(test, "attach multi: level1 size", 3 * MB,
			   level1_size_now);
		test_equal(test, "attach multi: level2 count", 2,
			   level2_count_now);
		...
		test_equal(test, "attach multi: 16KB at correct location",
			   5 * MB, buffer[0]->start);
		test_equal(test, "attach multi: 8KB at correct location",
			   2 * MB, buffer[1]->start);
		...
		test_null(test, "attach multi: didn't get small 1KB",
			  buffer[4]);

		test_equal(test, "attach multi: 2MB at correct location",
			   0 * MB, buffer[2]->start);
		test_equal(test, "attach multi: 2MKB at correct location",
			   4 * MB, buffer[3]->start);
	...
	test_group_start(test, "upper border regions");
	{
		...
		test_equal(test, "upper border regions: level1 count", 1,
			   level1_count_now);
		test_equal(test, "upper border regions: level1 size", 1 * MB,
			   level1_size_now);
		test_equal(test, "upper border regions: level1 count", 0,
			   level2_count_now);
		...
		test_not_null(test, "upper border regions: got 1MB at 4095MB",
			      buffer[0]);
		mreg_free(reg, buffer[0]);
		...
		test_equal(test, "Correct page start", 0xFFFF0000,
			   buffer[0]->start);
		test_equal(test, "Correct page start",
			   0xFFFF0000 + PAGE_SIZE - 1, buffer[0]->end);
	}
	test_group_end(test);
	...
	test_group_start(test, "lower border regions");
	{
		...
		test_equal(test, "lower border regions: level1 count", 0,
			   level1_count_now);
		test_equal(test, "lower border regions: level1 size", 0,
			   level1_size_now);
		test_equal(test, "lower border regions: level1 count", 1,
			   level2_count_now);
		...
		test_not_null(test, "lower border regions: got page 1",
			      buffer[0]);
		test_not_null(test, "lower border regions: got page 2",
			      buffer[1]);
		test_null(test, "lower border regions: didn't get page 3",
			  buffer[2]);
		...
		test_equal(test, "lower border regions (2): level1 count", 1,
			   level1_count_now);
		test_equal(test, "lower border regions (2): level1 size", MB,
			   level1_size_now);
		test_equal(test, "lower border regions (2): level1 count", 0,
			   level2_count_now);

		buffer[0] = mreg_alloc(reg, MB);
		buffer[1] = mreg_alloc(reg, PAGE_SIZE);
		test_not_null(test, "lower border regions (2): got page 1",
			      buffer[0]);
		test_null(test, "lower border regions (2): didn't get page 2",
			  buffer[1]);
		mreg_free(reg, buffer[0]);
		mreg_free(reg, buffer[1]);
		...
		buffer[2] = mreg_alloc(reg, PAGE_SIZE);
		test_not_null(test, "lower border regions (2B): got page 1",
			      buffer[0]);
		test_not_null(test, "lower border regions (2B): got page 2",
			      buffer[1]);
		test_null(test,
			  "lower border regions (2B): didn't get page 3",
			  buffer[2]);
		...
		test_equal(test, "lower border regions (3): level1 count", 1,
			   level1_count_now);
		test_equal(test, "lower border regions (3): level1 size", MB,
			   level1_size_now);
		test_equal(test, "lower border regions (3): level1 count", 1,
			   level2_count_now);
		...
		test_not_null(test, "lower border regions (3): got page 1",
			      buffer[0]);
		test_not_null(test, "lower border regions (3): got page 2",
			      buffer[1]);
		test_null(test, "lower border regions (3): didn't get page 3",
			  buffer[2]);
		mreg_free(reg, buffer[0]);
		mreg_free(reg, buffer[1]);
		mreg_free(reg, buffer[2]);
		...
		test_not_null(test, "lower border regions (3B): got page 1",
			      buffer[0]);
		test_not_null(test, "lower border regions (3B): got page 2",
			      buffer[1]);
		test_not_null(test, "lower border regions (3B): got page 3",
			      buffer[2]);
		test_null(test,
			  "lower border regions (3B): didn't get page 4",
			  buffer[3]);
	}
	test_group_end(test);
	...
	test_group_start(test, "alloc: stress test");
	{
		/* need a larger one */
		reg = mreg_create(PAGE_BITS);
		test_not_null(test, "sanity check: allocated a larger mreg",
			      reg);
