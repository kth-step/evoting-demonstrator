...
extern heap_marker_t *heap_marker_prev(heap_ctxt_t * heap,
				       heap_marker_t * mark);
extern heap_marker_t *heap_marker_next(heap_ctxt_t * heap,
				       heap_marker_t * mark);
extern void heap_marker_join(heap_ctxt_t * heap, heap_marker_t * m1,
			     heap_marker_t * m2);
extern heap_marker_t *heap_marker_split(heap_ctxt_t * heap,
...
void marker_dump(heap_marker_t * marker)
{
	if (!marker)
		...
		printf("  %c%c @%08lx  - Size %-8d (Prev %-8d) \n",
		       ...
		       marker_get_size(marker), marker_get_size_prev(marker)
		    );
	}
...
void test_do(test_context_t * ctx)
{
	...
	test_group_start(ctx, "Setup and sanity check");
	{
		...
		test_equal(ctx, "Initial block prev size is zero",
			   marker_get_size_prev(m0), 0);
		test_equal(ctx, "Initial block is free", marker_is_free(m0), 1);
		test_equal(ctx, "Initial block is last", marker_is_last(m0), 1);
		test_equal(ctx, "Only one block initially",
			   heap_count_markers(&heap), 1);
	}
	test_group_end(ctx);
	...
	test_group_start(ctx, "allocate");
	{
		...
		test_equal(ctx, "Has two blocks after allocation",
			   heap_count_markers(&heap), 2);

		m0 = marker_get_from_address(a0);
		test_larger_than(ctx, "Allocated at least 100 bytes",
				 marker_get_size(m0), 100);
		test_equal(ctx, "Allocated block is NOT free",
			   marker_is_free(m0), 0);
		...
		if (m1) {
			...
			test_equal(ctx, "prev allocated block is free",
				   marker_is_free(m1), 1);
			test_equal(ctx, "prev allocated block is NOT last",
				   marker_is_last(m1), 0);
			test_equal(ctx, "Allocated block is last",
				   marker_is_last(m0), 1);
			...
			test_equal(ctx, "next allocated block is free",
				   marker_is_free(m1), 1);
			test_equal(ctx, "next allocated block is last",
				   marker_is_last(m1), 1);

			/* before, so m0 must be the end */
			test_equal(ctx, "Allocated block is NOT last",
				   marker_is_last(m0), 0);
	...
	test_group_start(ctx, "allocate another");
	{
		...
		test_equal(ctx, "Has three blocks after another allocation",
			   heap_count_markers(&heap), 3);

		m1 = marker_get_from_address(a1);
		test_larger_than(ctx, "Allocated at least 100 bytes",
				 marker_get_size(m1), 100);
		test_equal(ctx, "Allocated block is NOT free",
			   marker_is_free(m1), 0);

		/* verify that a0 and a1 dont overlap */
		if (a0 > a1)
			test_larger_than(ctx, "a0 > a1 + 100 bytes",
					 (uint32_t) a0, (uint32_t) a1 + 100);
		else
			test_larger_than(ctx, "a1 > a0 + 100 bytes",
					 (uint32_t) a1, (uint32_t) a0 + 100);
	...
	{
		heap_free(&heap, a1);
		test_equal(ctx, "Has 2 blocks after first free",
			   heap_count_markers(&heap), 2);

		heap_free(&heap, a0);
		test_equal(ctx, "Has 1 block after second free",
			   heap_count_markers(&heap), 1);
		...
		test_equal(ctx, "Remaining block prev size is zero",
			   marker_get_size_prev(m0), 0);
		test_equal(ctx, "Remaining block is free", marker_is_free(m0),
			   1);
		test_equal(ctx, "Remaining block is last", marker_is_last(m0),
			   1);
