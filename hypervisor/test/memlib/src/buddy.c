...
void test_do(test_context_t * test)
{
	...
	{
		b = buddy_init(&buddy, MEM_BASE, MEM_SIZE, MEM_PAGE_BITS,
	...
	test_group_start(test, "blongs to");
	{
		...
		test_equal(test, "end-1 belongs to", TRUE,
			   buddy_belongs_to(&buddy,
					    MEM_BASE +
					    (MEM_PAGES_COUNT << MEM_PAGE_BITS)
					    - 1));
	...
	test_group_start(test, "buddy_get_order_from_size() ");
	{
		test_equal(test, "size = 1 page - 1", 0,
			   buddy_get_order_from_size(&buddy,
						     MEM_PAGE_SIZE - 1));
	...
	test_group_start(test, "allocate memory");
	{
		for (i = 0; i < MEM_PAGES_COUNT; i++) {
			...
			if (!b) {
				...
				test_equal(test,
					   "allocated memory should be page assigned",
					   0, mem[i] & MEM_PAGE_MASK);

				/* see if it is within the memory we have */
				test_larger_or_equal(test,
						     "allocated memory above base",
						     mem[i], MEM_BASE);

				test_less_or_equal(test,
						   "allocated memory below base + size",
						   mem[i],
						   MEM_BASE +
						   MEM_PAGE_SIZE *
						   MEM_PAGES_COUNT);
				...
				for (j = 0; j < i; j++)
					test_not_equal(test,
						       "Not returning same address twice",
						       mem[i], mem[j]);
			}
		}
	...
	test_group_start(test, "allocate at");
	{
		for (j = 0; j < 10; j++) {
			for (i = 0; i < 3; i++) {
				...
				test_equal(test, "allocate at address",
					   MEM_BASE, tmp);
	...
	test_group_start(test, "higher orders");
	{
		for (i = 0; i < 31; i++) {
			...
			if ((1UL << i) > MEM_PAGES_COUNT)
				...
				test_equal(test,
					   "Should not allocate at this high order",
					   FALSE, b);
			else
				test_equal(test, "Allocated higher order",
					   TRUE, b);
	...
	test_group_start(test, "stress test");
	{
		...
		for (r = 100000; r; --r) {
			for (i = 0; i < MEM_PAGES_COUNT; i++) {
				b = buddy_alloc(&buddy, 0, &mem[i]);
				test_equal(test, "stress test: alloc ok",
					   TRUE, b);
			}
			...
			b = buddy_alloc(&buddy, 0, &tmp);
			test_equal(test,
				   "stress test: no more blocks to allocate\n",
				   FALSE, b);
			...
			for (i = 0; i < MEM_PAGES_COUNT; i++) {
				b = buddy_free(&buddy, mem[i]);
				test_equal(test, "stress test freed memory",
					   TRUE, b);
			}
		}
	}
	...
	test_group_start(test, "first/last free");
	{
		...
		uint32_t last_adr =
		    MEM_BASE + MEM_PAGES_COUNT * MEM_PAGE_SIZE - 1;
		...
		for (i = 0; i < MEM_PAGES_COUNT; i++) {
			...
			if (i < MEM_PAGES_COUNT - 2) {
				test_equal(test, "first/last: alloc ok", TRUE,
					   b);

				test_larger_than(test,
						 "first/last: not allocating before first",
						 mem[i], first_adr);

				test_less_than(test,
					       "first/last: not allocating pas last",
					       mem[i], last_adr);
			} else {
				test_equal(test,
					   "first/last: alloc shoud fail",
					   FALSE, b);
			}
		}
	}
	...
	test_group_start(test, "border regions");
	{
		...
		for (i = 0; (1UL << (i + MEM_PAGE_BITS)) <= MB; i++) {
			...
			test_equal(test, "border_regions: normal alloc", TRUE,
				   b);
		...
		for (i = 0; (1UL << (i + MEM_PAGE_BITS)) <= MB; i++) {
			...
			test_equal(test, "border_regions: normal alloc_at",
				   TRUE, b);
			buddy_free(&buddy, mem[0]);
		}
		...
		for (i = 0;; i++) {
			...
			b = buddy_alloc_at(&buddy, start - size + MB, i,
					   &mem[0]);
			test_equal(test,
				   "border_regions: alloc_at at the end",
				   TRUE, b);
			buddy_free(&buddy, mem[0]);
		}
		...
		for (i = 0;; i += 2) {
			...
			b = buddy_alloc_at(&buddy, start - size * 2 + MB, i,
					   &mem[0]);
			test_equal(test,
				   "border_regions: alloc_at at the end (first)",
				   TRUE, b);

			b = buddy_alloc_at(&buddy, start - size + MB, i,
					   &mem[1]);
			test_equal(test,
				   "border_regions: alloc_at at the end (Second)",
				   TRUE, b);
