...
void test_do(test_context_t * test)
{
	...
	test_group_start(test, "memcpy");
	{
		for (i = 0; i < SIZE; i++) {
			...
			for (j = 0; j < SIZE; j++) {
				if (j < i)
					test_equal(test,
						   "memcpy got good data",
						   buffer1[j], buffer2[j]);
				else
					test_equal(test,
						   "memcpy didn't touch other data",
						   buffer1[j],
						   (char)(j | 0x80));
			}
		}
	...
	test_group_start(test, "memset");
	{
		for (i = 0; i < SIZE / 2; i++) {
			...
			for (j = 0; j < SIZE; j++) {
				if (j < i)
					test_equal(test, "memset set", i,
						   buffer1[j]);
				else
					test_equal(test, "memset untouched",
						   0x66, buffer1[j]);
			}
		}
	}
	...
	test_group_start(test, "memcmp");
	{
		...
		test_less_than(test, "memcmp (5)", memcmp("ABCD", "ABCE", 4),
			       0);
		test_larger_than(test, "memcmp (6)",
				 memcmp("ABCD", "ABCC", 4), 0);
	}
	test_group_end(test);
	...
	test_group_start(test, "strcpy");
	{
		for (i = 0; i < SIZE; i++) {
			...
			for (j = 0; j < SIZE; j++) {
				if (j <= i)
					test_equal(test,
						   "strcpy got good data",
						   buffer1[j], buffer2[j]);
				else
					test_equal(test,
						   "strcpy didn't touch other data",
						   buffer2[j], (char)0xFF);
			}
		}
	}
	...
	test_group_start(test, "strncpy");
	{
		...
		for (i = 0; i < 5; i++) {
			test_equal(test, "strcpy (2)", buffer[i], i < 4 ? "ABCD"[i] : '?');	/* last one is an untouched  */
		}
	}
	test_group_end(test);
	...
	test_group_start(test, "strcmp");
	{
		...
		test_not_equal(test, "strcmp(ABCD, ABC)", 0,
			       strcmp("ABCD", "ABC"));
		test_not_equal(test, "strcmp(ABC, ABCD)", 0,
			       strcmp("ABC", "ABCD"));
	}
	test_group_end(test);
